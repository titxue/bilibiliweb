    ...
    def getVideo(self,url):
        ...
        try:
            ...
            if videos["code"] == 0:
                ...
                # print("运行时间：{}".format(time.time()-self.times)) #打印开始到现在执行时间
                return video
...
class runthread(object):
    ...
    def run(self,number):
        ...
        maxs = [0]
        while True:
            for i in range(max(maxs),max(maxs)+number):
                aid.append(i)
            maxs.append(max(aid))
            mt = []
            for x in aid:
                t = Mythread(self.multi,(x),"") #创建线程
                # t.start()
                mt.append(t) #添加到线程队列
